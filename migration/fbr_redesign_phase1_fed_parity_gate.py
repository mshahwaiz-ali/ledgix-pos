...
"""Rollback-safe ERPNext-native FED parity proof.

This gate proves the current Ledgix FED fixture can be represented with
...
- FED is a per-unit native ERPNext ``On Item Quantity`` tax.
- Ledgix only maps the authoritative ERPNext tax account to the FBR
  ``FED Payable`` component for snapshot / payload translation.
...
not independently certify that the fixture's PKR 30/unit value is the legally
...
ITEM = "LEDGIX-P4-FED"
FED_ACCOUNT = "Ledgix P4 FED Payable - LEI"
...
def _new_item_tax_template() -> str:
    title = f"Ledgix FED Item Gate {frappe.generate_hash(length=8).upper()}"
    doc = frappe.get_doc(
        {
            ...
            "taxes": [
                ...
                {
                    "tax_type": FED_ACCOUNT,
                    "tax_rate": FED_PER_UNIT,
                    "not_applicable": 0,
                },
            ],
        }
...
def _new_sales_tax_template() -> str:
    title = f"Ledgix FED Gate {frappe.generate_hash(length=8).upper()}"
    doc = frappe.get_doc(
        {
            ...
            "taxes": [
                ...
                },
                {
                    ...
                    "account_head": FED_ACCOUNT,
                    "description": "ERPNext Native FED 30 per unit",
                    "rate": 0,
                    "included_in_print_rate": 0,
                },
            ],
    ...
    doc = frappe.get_doc(
        {
            ...
            "component": component,
            "active": 1,
            "notes": "Temporary Phase 1 FED parity mapping.",
        }
    )
    doc.insert(ignore_permissions=True)
...
def _case_evidence(doc) -> dict:
    ...
    return {
        **base,
        "fed_net_credit": _gl_net_credit(doc.name, FED_ACCOUNT),
        "tax_rows_by_account": [
            {
                "idx": row.idx,
...
def _snapshot_evidence(doc) -> dict:
    ...
    if len(result.get("lines") or []) != 1:
        frappe.throw("Expected one FED snapshot line.")
    ...
    fed_rows = [
        row
        for row in line.get("component_rows") or []
        if row.get("component") == "FED Payable"
        and row.get("account_head") == FED_ACCOUNT
    ]

    if len(gst_rows) != 1:
        frappe.throw("Expected one GST Sales Tax Applicable snapshot row.")
    if len(fed_rows) != 1:
        frappe.throw("Expected one FED Payable snapshot row.")

    return {
        "line": line,
        "gst_component": gst_rows[0],
        "fed_component": fed_rows[0],
...
def run() -> dict:
    ...
    if not frappe.db.exists("Account", FED_ACCOUNT):
        frappe.throw(f"Missing native FED Account {FED_ACCOUNT}.")
    ...
    try:
        ...
        if mapping.tax_basis != "Transaction Value":
            frappe.throw("FED fixture FBR mapping must use Transaction Value.")
        ...
        fed_component_name, fed_component_created = _ensure_component_mapping(
            FED_ACCOUNT,
            "FED Payable",
        )
        if fed_component_created:
            created.append(("Ledgix FBR Tax Component Mapping", fed_component_name))
        ...
        sale = erpnext_selling.build_sales_invoice(
            ...
            sale_channel="B2B",
            client_sale_id=f"NATIVE-GATE-FED-{frappe.generate_hash(length=10)}",
            allow_rate_override=False,
            update_stock=False,
            checkout_source="Phase 1 FED Parity Gate",
        )
        ...
        credit = erpnext_selling.create_sales_return(
            ...
            reason="Phase 1 FED parity rollback test",
            client_return_id=f"NATIVE-GATE-FED-RET-{frappe.generate_hash(length=10)}",
            checkout_source="Phase 1 FED Parity Gate",
        )
        credit.reload()
        ...
        sale_fed = sale_snapshot["fed_component"]
        return_gst = return_snapshot["gst_component"]
        return_fed = return_snapshot["fed_component"]

        sale_fed_rows = [
            row
            for row in sale_ev["tax_rows_by_account"]
            if row["account_head"] == FED_ACCOUNT
        ]
        return_fed_rows = [
            row
        ...
        checks = {
            ...
            "sale_fed_row_present": len(sale_fed_rows) == 1,
            "sale_fed_uses_on_item_quantity": len(sale_fed_rows) == 1
            and sale_fed_rows[0]["charge_type"] == "On Item Quantity",
            "sale_fed_amount_30": len(sale_fed_rows) == 1
            and _close(sale_fed_rows[0]["tax_amount"], FED_PER_UNIT),
            "sale_fed_gl_30": _close(
                sale_ev["fed_net_credit"], FED_PER_UNIT
            ),
            "sale_total_tax_210": _close(
                sale_ev["total_taxes_and_charges"], TOTAL_TAX
            ),
            "sale_grand_total_1210": _close(sale_ev["grand_total"], GRAND_TOTAL),
            ...
            "sale_snapshot_fed_30": _close(
                sale_fed.get("tax_amount"), FED_PER_UNIT
            ),
            "sale_snapshot_fed_component_30": _close(
                sale_snapshot["line"]["components"]["fed_payable"],
                FED_PER_UNIT,
            ),
            ...
            "return_fed_row_present": len(return_fed_rows) == 1,
            "return_fed_uses_on_item_quantity": len(return_fed_rows) == 1
            and return_fed_rows[0]["charge_type"] == "On Item Quantity",
            "return_fed_amount_minus_30": len(return_fed_rows) == 1
            and _close(return_fed_rows[0]["tax_amount"], -FED_PER_UNIT),
            "return_fed_gl_minus_30": _close(
                return_ev["fed_net_credit"], -FED_PER_UNIT
            ),
            "return_total_tax_minus_210": _close(
                return_ev["total_taxes_and_charges"], -TOTAL_TAX
            ),
            "return_grand_total_minus_1210": _close(
            ...
            "return_snapshot_fed_minus_30": _close(
                return_fed.get("tax_amount"), -FED_PER_UNIT
            ),
            "return_snapshot_fed_component_minus_30": _close(
                return_snapshot["line"]["components"]["fed_payable"],
                -FED_PER_UNIT,
            ),
            ...
            "fed_round_trip_zero": _close(
                sale_ev["fed_net_credit"] + return_ev["fed_net_credit"], 0
            ),
        }

        case = core._case(
            "fed_on_item_quantity",
            {
                "sale": sale_ev,
                ...
                "native_setup": {
                    ...
                    "fed_per_unit": FED_PER_UNIT,
                    "fed_account": FED_ACCOUNT,
                },
                "fbr_mapping": {
                    "name": mapping.name,
        ...
        result = {
            ...
            "scope_proven": [
                "FED fixture commercial value PKR 1,000",
                "ERPNext native GST 18% = PKR 180",
                "ERPNext native On Item Quantity FED = PKR 30/unit for this fixture",
                "ERPNext authoritative total tax = PKR 210",
                "ERPNext GST and FED GL accounts",
                "Native FBR collector maps authoritative FED amount",
                "Native full return reverses GST and FED",
                "No [LEDGIX-TAX] monetary rows",
            ],
            "scope_not_proven": [
                "Independent legal certification that PKR 30/unit is the correct live production FBR FED amount",
            ],
        }

        if failed_checks:
            frappe.throw(
                "FED native parity failed: " + ", ".join(failed_checks)
            )
    ...
    if not rollback_clean:
        ...
        frappe.throw(
            "FED accounting checks passed, but rollback verification "
