...
"""Rollback-safe ERPNext-native non-posting Sales Tax Withheld parity proof.

The current fixture expresses sales-tax-withheld-at-source as PKR 40/unit.
Unlike Extra/Further/FED, this amount must not become an additive Sales Invoice
tax row or GL posting.

Authority model proven by this gate:
- ERPNext Item Tax Template stores the rate against the reserved withholding account.
- The actual Sales Invoice financial tax rows contain GST only.
- Ledgix's native FBR collector asks ERPNext's own tax engine to calculate the
  non-posting `On Item Quantity` evidence amount.
- The withheld amount is therefore available for FBR evidence without changing
  Sales Invoice grand total, receivable, or GL.
"""
...
ITEM = "LEDGIX-P4-WITHHELD"
WITHHELD_ACCOUNT = "Ledgix P4 Sales Tax Withheld - LEI"

SALE_VALUE = 1000.0
GST_RATE = 18.0
GST_AMOUNT = 180.0
WITHHELD_PER_UNIT = 40.0
GRAND_TOTAL = 1180.0
...
def _new_price_list() -> tuple[str, str]:
    ...
    name = f"LEDGIX Withheld Gate {token}"
...
def _new_item_tax_template() -> str:
    title = f"Ledgix Withheld Item Gate {frappe.generate_hash(length=8).upper()}"
    doc = frappe.get_doc(
        {
            ...
            "taxes": [
                ...
                {
                    "tax_type": WITHHELD_ACCOUNT,
                    "tax_rate": WITHHELD_PER_UNIT,
                    "not_applicable": 0,
                },
            ],
        }
...
def _new_sales_tax_template() -> str:
    # Intentionally GST-only. The withheld account MUST NOT become a financial
    # Sales Invoice tax row.
    title = f"Ledgix Withheld Gate {frappe.generate_hash(length=8).upper()}"
    doc = frappe.get_doc(
        {
    ...
    doc = frappe.get_doc(
        {
            ...
            "component": component,
            "active": 1,
            "notes": "Temporary Phase 1 non-posting sales-tax-withheld parity mapping.",
        }
    )
    doc.insert(ignore_permissions=True)
...
def _case_evidence(doc) -> dict:
    ...
    return {
        **base,
        "withheld_gl_net_credit": _gl_net_credit(doc.name, WITHHELD_ACCOUNT),
        "financial_tax_accounts": [
            str(row.account_head or "") for row in doc.get("taxes") or []
        ],
    }
...
def _snapshot_evidence(doc) -> dict:
    ...
    if len(result.get("lines") or []) != 1:
        frappe.throw("Expected one Sales Tax Withheld snapshot line.")
    ...
    withheld_rows = [
        row
        for row in line.get("component_rows") or []
        if row.get("component") == "Sales Tax Withheld At Source"
        and row.get("account_head") == WITHHELD_ACCOUNT
    ]

    if len(gst_rows) != 1:
        frappe.throw("Expected one GST Sales Tax Applicable snapshot row.")
    if len(withheld_rows) != 1:
        frappe.throw("Expected one non-posting Sales Tax Withheld snapshot row.")

    return {
        "line": line,
        "gst_component": gst_rows[0],
        "withheld_component": withheld_rows[0],
...
def run() -> dict:
    ...
    if not frappe.db.exists("Account", WITHHELD_ACCOUNT):
        frappe.throw(f"Missing reserved Sales Tax Withheld Account {WITHHELD_ACCOUNT}.")
    ...
    def _no_network_queue(reference_doctype, reference_name, reason=None):
        ...
        return {
            "queued": False,
            "status": "Withheld Parity Gate Bypass",
    ...
    try:
        ...
        if mapping.tax_basis != "Transaction Value":
            frappe.throw("Withheld fixture FBR mapping must use Transaction Value.")
        ...
        withheld_component_name, withheld_component_created = _ensure_component_mapping(
            WITHHELD_ACCOUNT,
            "Sales Tax Withheld At Source",
        )
        if withheld_component_created:
            created.append(("Ledgix FBR Tax Component Mapping", withheld_component_name))
        ...
        sale = erpnext_selling.build_sales_invoice(
            ...
            sale_channel="B2B",
            client_sale_id=f"NATIVE-GATE-WITHHELD-{frappe.generate_hash(length=10)}",
            allow_rate_override=False,
            update_stock=False,
            checkout_source="Phase 1 Withheld Parity Gate",
        )
        ...
        credit = erpnext_selling.create_sales_return(
            ...
            reason="Phase 1 Withheld parity rollback test",
            client_return_id=f"NATIVE-GATE-WITHHELD-RET-{frappe.generate_hash(length=10)}",
            checkout_source="Phase 1 Withheld Parity Gate",
        )
        credit.reload()
        ...
        sale_withheld = sale_snapshot["withheld_component"]
        return_withheld = return_snapshot["withheld_component"]
        ...
        checks = {
            ...
            "sale_only_financial_tax_is_gst": sale_ev["financial_tax_accounts"]
            == [core.GST_ACCOUNT],
            "withheld_not_in_invoice_tax_rows": WITHHELD_ACCOUNT
            not in sale_ev["financial_tax_accounts"],
            "withheld_not_added_to_total_tax": _close(
                sale_ev["total_taxes_and_charges"], GST_AMOUNT
            ),
            "sale_grand_total_1180": _close(sale_ev["grand_total"], GRAND_TOTAL),
            "withheld_not_posted_to_gl": _close(
                sale_ev["withheld_gl_net_credit"], 0
            ),
            "sale_gl_balanced": sale_ev["gl_balanced"],
            "sale_no_legacy_tax_rows": not sale_ev["legacy_managed_rows"],
            "sale_snapshot_withheld_40": _close(
                sale_withheld.get("tax_amount"), WITHHELD_PER_UNIT
            ),
            "sale_snapshot_component_withheld_40": _close(
                sale_snapshot["line"]["components"]["sales_tax_withheld_at_source"],
                WITHHELD_PER_UNIT,
            ),
            "sale_snapshot_withheld_is_non_posting": bool(
                sale_withheld.get("non_posting_fbr_evidence")
            ),
            ...
            "return_withheld_not_in_invoice_tax_rows": WITHHELD_ACCOUNT
            not in return_ev["financial_tax_accounts"],
            "return_total_tax_only_gst": _close(
                return_ev["total_taxes_and_charges"], -GST_AMOUNT
            ),
            "return_grand_total_minus_1180": _close(
                return_ev["grand_total"], -GRAND_TOTAL
            ),
            "return_withheld_not_posted_to_gl": _close(
                return_ev["withheld_gl_net_credit"], 0
            ),
            "return_gl_balanced": return_ev["gl_balanced"],
            "return_no_legacy_tax_rows": not return_ev["legacy_managed_rows"],
            "return_snapshot_withheld_minus_40": _close(
                return_withheld.get("tax_amount"), -WITHHELD_PER_UNIT
            ),
            "return_snapshot_component_withheld_minus_40": _close(
                return_snapshot["line"]["components"]["sales_tax_withheld_at_source"],
                -WITHHELD_PER_UNIT,
            ),
            "return_snapshot_withheld_is_non_posting": bool(
                return_withheld.get("non_posting_fbr_evidence")
            ),
            "return_snapshot_reconciles": bool(
                return_snapshot["reconciliation"].get("passed")
            ),
            "withheld_snapshot_round_trip_zero": _close(
                sale_withheld.get("tax_amount")
                + return_withheld.get("tax_amount"),
                0,
            ),
        }

        case = core._case(
            "sales_tax_withheld_non_posting_erpnext_evidence",
            {
                "sale": sale_ev,
                ...
                "native_setup": {
                    ...
                    "withheld_item_tax_rate": WITHHELD_PER_UNIT,
                    "withheld_account": WITHHELD_ACCOUNT,
                    "withheld_financial_row": False,
                },
                "fbr_mapping": {
                    "name": mapping.name,
                    "needs_review_temporarily_cleared": True,
                    "money_field_source": (
                        "ERPNext Item Tax Template + ERPNext On Item Quantity calculation"
                    ),
                    "accounting_treatment": (
                        "Non-posting FBR evidence; not part of Sales Invoice total or GL"
                    ),
        ...
        result = {
            ...
            "scope_proven": [
                "Withheld fixture commercial value PKR 1,000",
                "ERPNext native GST 18% = PKR 180",
                "ERPNext native non-posting On Item Quantity withheld evidence = PKR 40/unit",
                "Sales Invoice grand total remains PKR 1,180",
                "No Sales Tax Withheld financial tax row",
                "No Sales Tax Withheld GL posting",
                "Native FBR collector exposes the ERPNext-calculated withheld amount",
                "Native full return reverses FBR withheld evidence without adding GL",
                "No [LEDGIX-TAX] monetary rows",
            ],
            "scope_not_proven": [
                "Independent legal certification that PKR 40/unit is the correct live production FBR withheld amount",
                "Payment settlement accounting when a buyer actually remits net of withheld sales tax",
            ],
        }

        if failed_checks:
            frappe.throw(
                "Sales Tax Withheld native parity failed: "
                + ", ".join(failed_checks)
            )
    ...
    if not rollback_clean:
        ...
        frappe.throw(
            "Sales Tax Withheld checks passed, but rollback verification found "
            "persisted gate state. No cleanup was attempted."
        )
