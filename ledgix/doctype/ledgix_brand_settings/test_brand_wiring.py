...
class TestBrandWiring(FrappeTestCase):
	def test_empty_settings_use_bundled_ledgix_identity(self):
		...
		self.assertTrue(brand.DEFAULT_SYMBOL_LOGO.endswith("ledgix-symbol.png"))
		self.assertTrue(brand.DEFAULT_FULL_LOGO.endswith("ledgix-lockup.png"))
		self.assertEqual(brand.DEFAULT_FAVICON_LOGO, brand.DEFAULT_SYMBOL_LOGO)
	...
	def test_frontend_wires_brand_setting_into_ledgix_surfaces(self):
		...
		self.assertIn("DEFAULT_FAVICON_LOGO = DEFAULT_SYMBOL_LOGO", brand_js)
	...
	def test_bundled_brand_assets_use_approved_png_identity(self):
		...
		self.assertEqual(
			asset_names,
			{"ledgix-symbol.png", "ledgix-lockup.png"},
		)
