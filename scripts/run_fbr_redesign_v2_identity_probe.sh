...
RESULT="$LOG_DIR/fbr-redesign-v2-identity-probe.txt"
BENCH_PYTHON="$ROOT_DIR/frappe-bench/env/bin/python"

if [[ "$SITE" != "ledgix-erpnext.local" ]]; then
  echo "[ERROR] Refusing V2 identity probe on site: $SITE" >&2
  exit 2
fi
